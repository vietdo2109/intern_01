  ...
  if (todo) {
    return (
      <Flex
        ...
        position={'relative'}
      >
        <UpdateTaskModal id={todo.id} todo={todo} onClose={onClose} isOpen={isOpen} />
        <Flex
          position={'absolute'}
          width={'180px'}
          zIndex={1000}
        ...
        <Flex justifyContent={'space-between'} padding={'18px'}>
          ...
          >
            <Icon as={HiOutlineDotsHorizontal} color={GRAY_COLOR} w={'20px'} h={'20px'}></Icon>
          </Box>
        </Flex>

        <Flex paddingX={'18px'} flexDir={'column'} gap={'20px'} flex={1} position={'relative'}>
          <Text fontWeight={'700'} fontSize={'24px'} height={'26px'}>
            {text}
          </Text>
          <Flex flex={1} alignItems={'center'} gap={'6px'} position={'absolute'} bottom={'10px'}>
            <Icon as={FaRegCalendarAlt} color={GRAY_COLOR}></Icon>
            <Text mt={'6px'} fontWeight={'700'} fontSize={'16px'} color={GRAY_COLOR}>
              {date}
            </Text>
          </Flex>
        </Flex>
